...
#define CLK   3          //旋转 ，不可更改
#define DT    2          //旋转 ，可更改其他引脚
...
boolean t12_kj_jr = 1;       //是否开机就加热？ 0-否 1-是

float t12_ad = 0.0;          //T12即时ADC值
uint16_t t12_temp = 0.0;     //T12即时温度
boolean t12_error = 0;       //手柄未接提示
boolean t12_adc_error = 0;   //温度超出测量范围
uint16_t set_temp_max;       //自动计算设置的温度最大值
uint16_t set_temp_min;       //自动计算设置的温度最小值
...
float vcc_refer = 4.95;                //参考电压，稳压芯片输出的电压是多少就填多少
float vin = 0.0;                       //电源电压
...
uint16_t sleep_time = 180;       //进入休眠的时间阈值,单位秒
...
#define sz_adc2  650
#define sz_adc3  940
uint16_t sz_temp[4] = {49, 262, 391, 486};   //深圳头曲线拟合的4段温度   Y
...
int16_t thisError_average = 0;    //平均值的误差
...
void setup()
{
  ...
  interrupts();                           // 开启全局中断
  //***** 打开外部中断
  attachInterrupt(digitalPinToInterrupt(CLK), xuan_zhaun, FALLING);  //PIN3，选择编码器 旋转，下降沿触发 FALLING
