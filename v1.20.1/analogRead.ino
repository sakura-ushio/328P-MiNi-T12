...
void read_t12_temp() //定时读取T12的温度
{
  ...
  //手柄检测及超出测量范围检测
  #define ADCMax 1000
  if (t12_switch == 0 && t12_ad >= ADCMax) //无手柄
  {
    t12_error = 1;
    t12_switch = 0;
  }
  else if (t12_switch == 1 && t12_ad >= ADCMax) //超出测量范围检测，延时检测
  {
    t12_adc_error_count++;
  ...
  else if (t12_switch == 1 && t12_ad < ADCMax)
  {
    t12_adc_error = 0;
    t12_adc_error_count = 0; //清除计数
  }
...
void auto_get_set_temp()   //自动获取设置温度的范围
{
  set_temp_max = ((sz_temp[3]) / 10 ) * 10 - 20;
  if (set_temp_max > 450) set_temp_max = 450;
