...
void xuan_zhaun()  //旋转中断 选框决策
{
  ...
  /*Serial.print("DT "); Serial.print(digitalRead(DT));
    ...
    Serial.println(" ");*/
  if ((millis() - xzbmq_time) < 20) return; //旋转响应间隔 20ms
  buzzer(100, 70); //时间，次数
...
void sleep_xz_key()  //设置界面 旋转
{
  ...
  {
    if (digitalRead(DT) == xzbmq_fx)      sleep_time += 30;
    else if (digitalRead(CLK) == xzbmq_fx) sleep_time -= 30;
  }
  ...
  else if (sleep_time > 999) sleep_time = 30;
  else if (sleep_time < 30) sleep_time = 999;
}
//************************ OLED亮度设置界面 ************************
